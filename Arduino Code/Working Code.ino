...
// Enhanced ultrasonic sensor diagnostics
struct UltrasonicDiagnostics {
  ...
  unsigned long minDuration = 99999;
...
void setup() {
  ...
  Serial.println(F("Ultrasonic Debug: ULTRA_TEST, ULTRA_STATS, ULTRA_RESET"));
  
  // Enhanced sensor tests with detailed diagnostics
  Serial.println(F("=== ENHANCED SENSOR TEST ==="));
  performDetailedUltrasonicTest();
  Serial.println(F("============================="));
  
  Serial.flush();
}

// Enhanced ultrasonic sensor test with detailed diagnostics
void performDetailedUltrasonicTest() {
  Serial.println(F("?? Testing ultrasonic sensor (HC-SR04)..."));
  Serial.println(F("Pin configuration: TRIG->2, ECHO->3"));
  
  // Test multiple readings to see consistency
  for (int i = 1; i <= 5; i++) {
    Serial.print(F("Test "));
    Serial.print(i);
    Serial.print(F(": "));
    
    int distance = getDistanceWithDiagnostics();
  ...
  Serial.print(F("Initial stats - Timeouts: "));
  Serial.print(ultrasonicStats.timeoutCount);
  Serial.print(F("/"));
  Serial.println(ultrasonicStats.totalReadings);
}
...
void processSerialCommand() {
  if (Serial.available()) {
    ...
    if (command == "PING") {
    ...
    else if (command == "ULTRA_TEST") {
      performDetailedUltrasonicTest();
    }
    else if (command == "ULTRA_STATS") {
      printUltrasonicStats();
    }
    else if (command == "ULTRA_RESET") {
      resetUltrasonicStats();
      Serial.println(F("Ultrasonic statistics reset"));
    }
      ...
      if (distance == -1) {
        Serial.println(F("SENSOR COVERED (Food Available)"));
      } else {
        Serial.print(distance);
        Serial.print(F("cm"));
      ...
      Serial.println(F("Distance Thresholds: <7cm=Food Available, >=7cm=Empty/Low, TIMEOUT=Food Available (Covered)"));
      
      // Add ultrasonic sensor stats to status
      Serial.println(F("--- Ultrasonic Sensor Stats ---"));
      Serial.print(F("Total readings: "));
      Serial.println(ultrasonicStats.totalReadings);
      Serial.print(F("Timeout rate: "));
      if (ultrasonicStats.totalReadings > 0) {
        float timeoutPercent = (float)ultrasonicStats.timeoutCount / ultrasonicStats.totalReadings * 100;
        Serial.print(timeoutPercent, 1);
        Serial.println(F("%"));
      } else {
        Serial.println(F("N/A"));
      }
      Serial.print(F("Last valid distance: "));
      if (ultrasonicStats.lastValidDistance >= 0) {
        Serial.print(ultrasonicStats.lastValidDistance);
        Serial.println(F("cm"));
      } else {
      ...
      // Get immediate distance reading with enhanced diagnostics
      Serial.println(F("=== DISTANCE DEBUG ==="));
      int distance = getDistanceWithDiagnostics();
      if (distance != -1) {
        Serial.print("Distance: ");
        Serial.print(distance);
...
void setStorageState(StorageState state, int distance) {
  if (state == STORAGE_ADEQUATE) {
    if (distance == -1) {
      Serial.print(F("Food Available: SENSOR COVERED (Food completely covers sensor)"));
      Serial.println();
    } else {
...
void loop() {
  ...
  if (distance == -1) {
    // UPDATED LOGIC: Timeout/Error when completely covered = Food Available
    // This happens when food completely covers the sensor and absorbs sound waves
  ...
  if (millis() - lastDbg > 20000) {
    ...
    if (distance == -1) {
      Serial.println(F("COVERED (Food Available)"));
    } else {
      Serial.print(distance);
      Serial.print(F("cm"));
...
// Enhanced getDistance function with detailed diagnostics
int getDistanceWithDiagnostics() {
  ultrasonicStats.totalReadings++;
  
  // Clear any previous echo
  ...
  // Measure echo pulse duration with extended timeout
  unsigned long duration = pulseIn(echoPin, HIGH, 35000UL); // Increased timeout to 35ms
  
  // Update duration statistics
  if (duration > 0) {
    if (duration < ultrasonicStats.minDuration) ultrasonicStats.minDuration = duration;
    if (duration > ultrasonicStats.maxDuration) ultrasonicStats.maxDuration = duration;
    
    // Calculate running average
    ultrasonicStats.averageDuration = ((ultrasonicStats.averageDuration * (ultrasonicStats.totalReadings - 1)) + duration) / ultrasonicStats.totalReadings;
  }
  ...
  if (duration == 0) {
    ultrasonicStats.timeoutCount++;
    ultrasonicStats.consecutiveTimeouts++;
    Serial.println(F("(TIMEOUT)"));
    
    if (ultrasonicStats.consecutiveTimeouts >= 3) {
      Serial.println(F("?? Multiple consecutive timeouts! Check wiring:"));
      Serial.println(F("  TRIG pin 2 -> HC-SR04 Trig"));
      Serial.println(F("  ECHO pin 3 -> HC-SR04 Echo"));
  ...
  ultrasonicStats.consecutiveTimeouts = 0; // Reset consecutive timeout counter
  ...
  if (distCm <= 0) {
    ultrasonicStats.invalidCount++;
    Serial.println(F("(INVALID: ?0)"));
    return -1;
  } else if (distCm > 400) {
    ultrasonicStats.invalidCount++;
    Serial.println(F("(INVALID: >400cm)"));
    return -1;
  } else if (distCm < 2) {
    // Very close reading - might be interference
    Serial.print(F("(VERY CLOSE - possible interference) "));
    ultrasonicStats.lastValidDistance = (int)distCm;
    ultrasonicStats.lastSuccessfulReading = millis();
    return (int)distCm;
  } else {
    // Valid reading
    ultrasonicStats.lastValidDistance = (int)distCm;
...
// Regular getDistance function (non-diagnostic)
int getDistance() {
  digitalWrite(trigPin, LOW);
  delayMicroseconds(2);
  digitalWrite(trigPin, HIGH);
...
void printUltrasonicStats() {
  ...
  Serial.println(ultrasonicStats.invalidCount);
  Serial.print(F("Consecutive timeouts: "));
  Serial.println(ultrasonicStats.consecutiveTimeouts);
  Serial.print(F("Last valid distance: "));
  if (ultrasonicStats.lastValidDistance >= 0) {
    Serial.print(ultrasonicStats.lastValidDistance);
    Serial.println(F("cm"));
  ...
  if (ultrasonicStats.totalReadings > 0) {
    Serial.print(F("Min duration: "));
    Serial.print(ultrasonicStats.minDuration);
    Serial.println(F("?s"));
    Serial.print(F("Max duration: "));
    Serial.print(ultrasonicStats.maxDuration);
    Serial.println(F("?s"));
    Serial.print(F("Avg duration: "));
    Serial.print(ultrasonicStats.averageDuration, 1);
    Serial.println(F("?s"));
  }
  
  Serial.print(F("Time since last valid reading: "));
  if (ultrasonicStats.lastSuccessfulReading > 0) {
    unsigned long timeSince = millis() - ultrasonicStats.lastSuccessfulReading;
...
void resetUltrasonicStats() {
  ultrasonicStats.totalReadings = 0;
  ultrasonicStats.timeoutCount = 0;
  ultrasonicStats.invalidCount = 0;
  ultrasonicStats.lastSuccessfulReading = 0;
  ultrasonicStats.consecutiveTimeouts = 0;
  ultrasonicStats.lastValidDistance = -1;
  ultrasonicStats.minDuration = 99999;
  ultrasonicStats.maxDuration = 0;
  ultrasonicStats.averageDuration = 0;
}